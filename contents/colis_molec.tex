...
\begin{minipage}{0.5\textwidth}
  ...
  % \centering \includegraphics[width=.8\linewidth]{figures/coordenadas}
    \caption{Coordenadas utilizadas}
\end{minipage}\hfill
\begin{minipage}{0.4\textwidth}
  \begin{subequations}
    \begin{align}
      &\vect{r}=  \bm{r}_{\text{{P}}} - \bm{r}_{X} , \\
...
\begin{align*}
  R_{j}(t) - R_{j}^{\text{eq}}&= \sum_{\alpha=1}^{15} A_{\alpha}\, q^{(\alpha)}_{j} \cos{(\omega^{(\alpha)} t + \phi_{\alpha})} \\
